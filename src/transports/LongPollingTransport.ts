...
export default class LongPollingTransport extends Transport {
  ...
  private _pollTimeoutId = null;
  private _maxReconnectedTimeout: number;
  private _url: string;
  private _currentTimeoutId: number | undefined = undefined;
  ...
  _poll() {
    const poll = () => {
      const { messageId, groupsToken, shouldReconnect } = this._lastMessage;
      this._current = request.post(this._url + '/poll');
      this._current = this._queryData(this._current);
      if (groupsToken) {
        this._current = this._current.send({ messageId, groupsToken });
      } else {
        this._current = this._current.send({ messageId });
      }
      this._current = this._current.end((err: any, res: any) => {
